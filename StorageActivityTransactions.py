...
def main():
    ...
    # Handle null values and filter records - keep only records with non-null essential fields
    ...
    # Auto-generate new UUID for records with null ActivityId
    import uuid
    uuid_udf = udf(lambda: str(uuid.uuid4()), StringType())
    trans_stream_df = trans_stream_df.withColumn(
        "ActivityId",
        when(col("ActivityId").isNull(), uuid_udf()).otherwise(col("ActivityId"))
    )
