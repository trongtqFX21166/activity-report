...
def create_spark_session():
    ...
    return SparkSession.builder \
        .appName("membership-monthly-redis-sync") \
...
def generate_document_id(phone, month, year):
    """Generate Redis document ID following the index pattern"""
    return f"activity-dev:membership:monthly:{year}:{month}:{phone}"
        ...
        for i in range(0, len(records), sub_batch_size):
            ...
            for record in sub_batch:
                try:
                    ...
                    doc_id = generate_document_id(record['phone'], record['month'], record['year'])
                    ...
                    pipeline.sadd("activity-ranking-monthly-idx", doc_id)
                    sub_batch_inserts += 1
            ...
            try:
                ...
                for record in sub_batch:
                    try:
                        ...
                        redis_client.sadd("activity-ranking-monthly-idx", doc_id)
                        inserts += 1
...
def process_data(spark, redis_client, month=None, year=None):
    ...
    try:
        ...
        print(f"Starting data processing for {month}/{year}...")
...
def main():
    ...
    current_date = datetime.now()
    if month and year:
        print(f"Starting Membership Monthly Redis Sync Job at {current_date} for {month}/{year}")
    else:
        print(f"Starting Membership Monthly Redis Sync Job at {current_date} for current month")
    ...
    try:
        ...
        process_data(spark, redis_client, month, year)
