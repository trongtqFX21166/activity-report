...
def get_membership_mapping() -> object:
    ...
    return [
        ...
        (0.01, 0.05, "Level4", "Nổi tiếng"),  # Top 4%
        (0.05, 0.10, "Level3", "Uy tín"),  # Top 10%
    ]
...
def assign_membership_levels(df):
    ...
    # Apply membership rules based on percentiles first
    ...
    level2_condition = when(
        col("totalpoints") >= 200,
        struct(lit("Level2").alias("code"), lit("Ngôi sao đang lên").alias("name"))
    )
    membership_conditions.append(level2_condition)

    # Default to Level1 for everyone else
    level1_condition = when(
        lit(True),
        struct(lit("Level1").alias("code"), lit("Nhập môn").alias("name"))
    )
    membership_conditions.append(level1_condition)

    # Apply all conditions with coalesce to prioritize them in order
...
def update_membership_status(month=None, year=None, env='dev'):
    ...
    try:
        ...
        # Read specified month's data from MongoDB using the dedicated function
