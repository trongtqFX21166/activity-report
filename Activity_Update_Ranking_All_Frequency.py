...
def check_dag_status(**context):
    ...
    if failed_tasks or dag_run.state == State.FAILED:
        ...
        print(f"DAG {dag_id} completed successfully. No notification sent as requested.")
...
with DAG('Activity_Update_Ranking_All_Frequency', default_args=default_args,
         schedule="21 0 * * *", catchup=False, dagrun_timeout=timedelta(minutes=60)) as dag:
    ...
    Load_Redis_AllTime = SparkSubmitOperator(
        task_id="Load_Redis_AllTime",
        conn_id="spark_prod_batch_report",
    ...
    begin_task >> Ranking_Profile_AllTime >> Load_Redis_AllTime >> end_task
