...
def fetch_profiles(conn):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            cursor.execute(
                'SELECT "Id", "Phone", "TotalPoints", "Level", "MembershipCode" FROM public."Profiles"'
            )
...
def main():
    """Main entry point"""
    logger.info("Starting Profile Level Upgrade Batch Process")

    try:
        # Log the level thresholds for verification
